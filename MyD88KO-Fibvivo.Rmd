#### First we load all the neccessary packages
...
coldata$group[grepl("G2", rownames(coldata))] <- "MyD88KO-Fib"
```
...
```{r}
highlightdwn<- ctrl.vs.ko[rownames(ctrl.vs.ko) %in% c("Spp1","Ccl2", "Il6st", "Icam1", "Cxcr5", "Ccl24",  "Il1b","C3", "Cxcl2", "Cxcr2", "C7", "Lyz2", "S100a8", "S100a9"), ]
...
ggplot(ctrl.vs.ko, aes(x = log2FoldChange, y = -log10(pvalue))) + 
  ...
  labs(x = "log2(fold change)",
       y = "-log10(P-value)",
       colour = "Expression \nchange") +
  annotate("text", x = -4, y = 25, label = sprintf("Up in Control: %d", sum(ctrl.vs.ko$sign == "Downregulated"))) +
  annotate("text", x = 4, y = 25, label = sprintf("Up in MyD88-KO: %d", sum(ctrl.vs.ko$sign == "Upregulated"))) + 
...
```{r}
write.csv(gse, "gse-ctrl-vs-ko.csv")
```
...
dotplot(gse, 
 ...
 categorys <- c("leukocyte chemotaxis",  "regulation of inflammatory response", "regulation of leukocyte activation", "leukocyte proliferation", "phagocytosis","production of molecular mediator of immune response", "positive regulation of leukocyte activation", "detection of stimulus", "regulation of defense response", "interleukin-6 production",
