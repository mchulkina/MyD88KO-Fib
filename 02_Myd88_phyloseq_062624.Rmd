...
ps1.colon.cecal.rarefied <-  ps1.rarefied %>% subset_samples(Isolation.Source %in% c("ceacal_content", "colon_content"))
```
...
```{r}
pdf("./Plots/p_alphaobj.colon.plots.pdf")
...
ord.pcoa.wuni <- ordinate(ps1.colon.rarefied, method = "PCoA", distance = "wunifrac")

ord.pcoa.wuni.cecal <-  ordinate(ps1.cecal.rarefied, method = "PCoA", distance = "wunifrac")


# Unifrac
#p.pcoa.uni <- plot_ordination(ps1.colon.rarefied, ord.pcoa.uni, shape = "Isolation.Source", color = "Treatment", axes = c(1,2)) +
...
p.pcoa.wuni <- plot_ordination(ps1.colon.rarefied, ord.pcoa.wuni, color = "Treatment", axes = c(1,2)) +
  geom_point(size = 2) + scale_color_manual(values = c("steelblue", "firebrick")) +
...
```{r community-composition-plots - Colon content only}
ps1.colon.rarefied.mpse
```
...
ps1.colon_phylum <- ps1.colon.rarefied  %>%
...
write.table(ps1.colon_phylum, "./Tables/Colon_content_phyla_relativeAbundance.tsv", sep='\t', quote=F, col.names = F, row.names = F)


p.ps1.colon_phylum <- ggplot(ps1.colon_phylum, aes(x = Sample_name, y = Abundance, fill = Phylum)) + 
  geom_bar(stat = "identity", width = 0.8, ) +
  facet_wrap(~Treatment, scales = "free_x", nrow = 4, ncol = 7) +
  theme(axis.text.x = element_blank()) +
  theme(axis.title.x = element_blank()) +
  labs(title = "Relative Abundant Phylum in Colon Content") + 
...
# Create a data table for ggplot
ps1.cecal_phylum <- ps1.cecal.rarefied  %>%
  tax_glom(taxrank = "Phylum") %>%                      # agglomerate at phylum level
  transform_sample_counts(function(x) {x/sum(x)} ) %>% # Transform to rel. abundance (or use ps0.ra)
  psmelt()                                    # Melt to long format for easy ggploting

write.table(ps1.cecal_phylum, "./Tables/Cecal_content_phyla_relativeAbundance.tsv", sep='\t', quote=F, col.names = F, row.names = F)


p.ps1.cecal_phylum <- ggplot(ps1.cecal_phylum, aes(x = Sample_name, y = Abundance, fill = Phylum)) + 
  geom_bar(stat = "identity", width = 0.8, ) +
  facet_wrap(~Treatment, scales = "free_x", nrow = 4, ncol = 7) +
  theme(axis.text.x = element_blank()) +
  theme(axis.title.x = element_blank()) +
  labs(title = "Relative Abundant Phylum in Cecal Content") + 
  scale_fill_manual(values=colors) +
  theme(
    legend.key.size = unit(0.5, "cm"),   # Reduce the size of the legend key (box)
    legend.key.height = unit(0.5, "cm"), 
...
taxa_table <- phyloseq::tax_table(ps1.colon.rarefied)
sample_data <- sample_data(ps1.colon.rarefied)

```
```{r}
# Create a data table for ggplot
#ps1.colon.rarefied.ra <- transform_sample_counts(ps1.colon.rarefied, function(x) x / sum(x))

phylum_ps1.colon.rarefied <- ps1.colon.rarefied %>% tax_glom(taxrank = "Phylum") %>% 
        transform_sample_counts(function(x) {x/sum(x)}) %>% psmelt()
phylum_ps1.colon.rarefied
...
b_data_ps <- subset_taxa(ps1.colon.rarefied, Phylum=="Bacteroidetes")
...
# Create a data table for ggplot
#ps1.cecal.rarefied.ra <- transform_sample_counts(ps1.cecal.rarefied, function(x) x / sum(x))
...
pdf('./Plots/Cecal.Bact_Firm_Ratio_phy.pdf')
p_bf
dev.off()
...
p_pf <- ggplot(merged_data_pf, aes(x = Treatment_Firmicutes, y = Prot_Firm_Ratio, fill = Treatment_Firmicutes)) +
  ...
  annotate("text", x = 1.5, y = 0.09,
           ...
           hjust = 0.5, vjust = 0, size = 5)
pdf('./Plots/Cecal.Prot_Firm_Ratio_phy.pdf')
p_pf
dev.off()
```
...
p_vf <- ggplot(merged_data_vf, aes(x = Treatment_Firmicutes, y = Verr_Firm_Ratio, fill = Treatment_Firmicutes)) +
  ...
  annotate("text", x = 1.5, y = 0.5,
           ...
           hjust = 0.5, vjust = 0, size = 5)
pdf('./Plots/Cecal.Verr_Firm_Ratio_phy.pdf')
p_vf
dev.off()
```
...
```{r}
wilcox_test_result_vf$p.value

#0.6857143
```

```{r}
pdf('./Plots/cecal.all_Ratio_phy.pdf')
p_bf / p_pf / p_vf  
dev.off()
```
...
ggplot(data = ., aes(x = Treatment, y = Abundance)) +
  ...
  facet_wrap(~ Phylum, scales = "free") + theme(axis.text.x = element_text(angle = 25, hjust = 1, size = 8))
...
permanova_all <- doadonis(ps1.colon.cecal.rarefied, "Treatment")
write.table(permanova_all$aov.tab, "./permanova_all.txt")

# Trivial example as it considers a subset of samples
...
### 5.3 Permutational Multivariate Analysis of Variance
#### We also can perform the Permutational Multivariate Analysis of Variance using the vegan

```{r}
distme <- get_dist(ps1.log, distmethod ="bray", method="hellinger")
sampleda <- data.frame(sample_data(ps1.log), check.names=FALSE)
sampleda <- sampleda[match(colnames(as.matrix(distme)),rownames(sampleda)),,drop=FALSE]
sampleda$Treatment <- factor(sampleda$Treatment)
set.seed(1024)
adores <- adonis(distme ~ Treatment, data=sampleda, permutation=9999)
...
conditions <- sample_data(ps1.colon.rarefied)$Treatment
tse <- otu_table(ps1.colon.rarefied)
x <- aldex(
  reads = otu_table(ps1.colon.rarefied),
  conditions = conditions,
  # 128 recommened for ttest, 1000 for rigorous effect size calculation
  mc.samples = 128, test="t", effect=TRUE,
  denom = "all",
  include.sample.summary=TRUE, verbose=TRUE, paired.test=FALSE)
...
```{r}
par(mfrow=c(1,2))
aldex.plot(x, type="MA", test="welch", xlab="Log-ratio abundance",
    ylab="Difference")
...
```{r}
genus_ps1.colon.rarefied <- ps1.colon.rarefied %>% tax_glom(taxrank = "Genus") 

mm_aldex_genus <- run_aldex(
     genus_ps1.colon.rarefied,
     group = 'Treatment',
...
pdf('./Plots/aldex_2_Genus_cladogram.pdf')
...
```{r}
pdf('./plots/aldex_heatmap_genus.pdf')
...
```{r}
colors <- c('#F7911E', '#767BBB', '#908E35', '#915323', '#FDD379', '#D1D1CC','#3D57A7', '#EE3324', '#F7921E',  '#F6EC21', '#79797A', 'grey', 'steelblue')
p_genus <- ps1.colon.rarefied.mpse %>%
         mp_plot_abundance(
...
colors <- c('#F0C18A','#008D47','#767BBB','#00AEAA','blue' , '#908E35', '#915323', '#FDD379', '#D1D1CC','#F5EB00', '#00ABD2', 
             '#A0489B','#F26822','#F175AD', '#919799', '#B52026', '#BCAED5', '#3E8386','#CCCC64' ,
            '#5793CE', '#8D3E1E', '#6591CC', '#986627', '#4D622C', '#D75345', '#483987' , '#951B53', '#696A6C')


# Create a data table for ggplot
ps1_genus <- ps1.colon.rarefied %>%
  tax_glom(taxrank = "Genus") %>%                      # agglomerate at phylum level
  transform_sample_counts(function(x) {x/sum(x)} ) %>% # Transform to rel. abundance (or use ps0.ra)
  psmelt() %>%                                         # Melt to long format for easy ggploting
  filter(Abundance > 0.0005)                             # Filter out low abundance taxa

write.table(ps1_genus, "./Tables/Colon_content_relativeAbundance_genus.tsv", sep='\t', quote=F, row.names = F, col.names = F)
# Plot - Genus

p.ps1_genus <- ggplot(ps1_genus, aes(x = Sample_name, y = Abundance, fill = Genus)) + 
  geom_bar(stat = "identity", width = 0.8, ) +
  facet_wrap(~Treatment, scales = "free_x", nrow = 4, ncol = 7) +
  theme(axis.text.x = element_blank()) +
  theme(axis.title.x = element_blank()) +
...
```{r}
# Create a data table for ggplot
colors <- c('#F0C18A','#008D47','blue','#767BBB','#00AEAA' , '#908E35', '#915323', '#FDD379', '#D1D1CC','#3D57A7', '#00ABD2', 
            '#F5EB00', '#A0489B','#F26822','#F175AD', '#919799', '#B52026', '#BCAED5', '#3E8386','#CCCC64' ,
            '#5793CE', '#8D3E1E', '#6591CC', '#986627', '#4D622C', '#D75345', '#483987' , '#951B53', '#696A6C')

ps1_genus.0.005 <- ps1.colon.rarefied %>%
  tax_glom(taxrank = "Genus") %>%                      # agglomerate at phylum level
  transform_sample_counts(function(x) {x/sum(x)} ) %>% # Transform to rel. abundance (or use ps0.ra)
  psmelt() %>%                                         # Melt to long format for easy ggploting
...
p.ps1_genus.0.005 <- ggplot(ps1_genus.0.005, aes(x = Sample_name, y = Abundance, fill = Genus)) + 
  ...
  theme(axis.text.x = element_blank()) +
  theme(axis.title.x = element_blank()) +
  labs(title = "Realtive Abundant Genus in Colon Content (> 0.5%)") + 
  scale_fill_manual(values=colors) +
  theme(
    legend.key.size = unit(0.3, "cm"),   # Reduce the size of the legend key (box)
    legend.key.height = unit(0.3, "cm"), 
...
pdf("./Plots/Colon_relative_plot_genus_0.5.pdf")
...
```{r}
ps1_genus ## This is phyloseq of Colon content

# Filter data for Bacteroides , Roseburis , Escherichia, Roseburia, Parabacteroides 
colon_bacteroides_data <- ps1_genus %>%
  filter(Genus == "Bacteroides")

colon_escherichia_data <- ps1_genus %>%
  filter(Genus == "Escherichia/Shigella")

colon_roseburia_data <- ps1_genus %>%
  filter(Genus == "Roseburia")

colon_parabacteroides_data <- ps1_genus %>%
...
```{r}
# Perform Wilcoxon rank-sum test
wilcox_test_result_Bacteroides_Roseburia <- wilcox.test(Bact_Rose_Ratio ~ Treatment_Roseburia, data = colon_merged_data_Bacteroides_Roseburia)

# Create the ggplot
p_bf <- ggplot(colon_merged_data_Bacteroides_Roseburia, aes(x = Treatment_Roseburia, y = Bact_Rose_Ratio, fill = Treatment_Roseburia)) +
  geom_boxplot() +
  labs(x = NULL, y = "Bacteroides/Roseburia") +
  theme_minimal() + 
  scale_fill_manual(name = "Treatment", values = alpha(c("steelblue", "firebrick"), alpha=0.9)) +
  ...
  annotate("text", x = 1.5, y = 0.65,
           label = sprintf("p = %.4f", wilcox_test_result_bf$p.value),
           hjust = 0.5, vjust = 0, size = 5)
pdf('./Plots/Colon.Bacteroides_Roseburia_Ratio_genus.pdf')
p_bf
dev.off()
...
# Perform Wilcoxon rank-sum test
wilcox_test_result_Escherichia_Roseburia <- wilcox.test(Escherichia_Roseburia_Ratio ~ Treatment_Roseburia, data = colon_merged_data_Escherichia_Roseburia)

# Create the ggplot
p_bf <- ggplot(colon_merged_data_Escherichia_Roseburia, aes(x = Treatment_Roseburia, y = Escherichia_Roseburia_Ratio, fill = Treatment_Roseburia)) +
  geom_boxplot() +
  labs(x = NULL, y = "Escherichia/Roseburia") +
  theme_minimal() + 
  scale_fill_manual(name = "Treatment", values = alpha(c("steelblue", "firebrick"), alpha=0.9)) +
  ...
  annotate("text", x = 1.5, y = 0.65,
           label = sprintf("p = %.4f", wilcox_test_result_bf$p.value),
           hjust = 0.5, vjust = 0, size = 5)
pdf('./Plots/Colon.Escherichia_Roseburia_Ratio_genus.pdf')
p_bf
dev.off()
...
# Perform Wilcoxon rank-sum test
wilcox_test_result_Parabacteroides_Bacteroides<- wilcox.test(Para_Bact_Ratio ~ Treatment_Bacteroides, data = colon_merged_data_Parabacteroides_Bacteroides)

# Create the ggplot
p_bf <- ggplot(colon_merged_data_Parabacteroides_Bacteroides, aes(x = Treatment_Bacteroides, y = Para_Bact_Ratio, fill = Treatment_Bacteroides)) +
  geom_boxplot() +
  labs(x = NULL, y = "Parabacteroides/Bacteroides") +
  theme_minimal() + 
  scale_fill_manual(name = "Treatment", values = alpha(c("steelblue", "firebrick"), alpha=0.9)) +
  ...
  annotate("text", x = 1.5, y = 0.65,
           label = sprintf("p = %.4f", wilcox_test_result_bf$p.value),
           hjust = 0.5, vjust = 0, size = 5)
pdf('./Plots/Colon.Parabacteroides_Bacteroides_genus.pdf')
p_bf
dev.off()
```
